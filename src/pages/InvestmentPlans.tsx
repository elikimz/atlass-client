...
export default function InvestmentPlans() {
  ...
  const getPlanColors = (name: string) => {
    ...
    if (name.includes('LV3')) return { bg: '#F59E0B', header: '#D97706', btn: '#1D4ED8' };
  ...
  return (
    <div style={{ 
      ...
      <div style={{ backgroundColor: 'var(--bg-card)', padding: '16px', borderBottom: '1px solid var(--border-main)', borderLeft: '1px solid var(--border-main)', borderRight: '1px solid var(--border-main)' }}>
        ...
        <div style={{ backgroundColor: 'var(--bg-main)', padding: '8px 12px', borderRadius: '6px', border: '1px solid var(--border-main)', fontSize: '12px', fontWeight: 600, color: 'var(--text-main)' }}>
          ACTIVE PLAN: <span style={{ color: 'var(--accent-primary)' }}>{user?.current_plan?.name || 'None'}</span> | VALIDITY: {isExpired ? '⚠️ EXPIRED' : (user?.current_plan?.validity_days || 0) + ' Days'}
        </div>
      </div>
      ...
      {message && (
        <div style={{ 
          ...
          backgroundColor: message.type === 'success' ? 'rgba(0, 172, 79, 0.1)' : 'rgba(220, 38, 38, 0.1)',
          color: message.type === 'success' ? '#00AC4F' : '#DC2626',
