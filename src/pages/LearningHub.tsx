...
const guidelinesPages = [
  {
    ...
    content: (
      <div>
        ...
        <div style={{ backgroundColor: '#fef2f2', border: '1px solid #fecaca', borderRadius: '8px', padding: '14px 16px', marginBottom: '16px' }}>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '13px', color: '#7f1d1d', margin: 0, lineHeight: 1.5 }}>Do not skip events or label them incorrectly. Every segment must be labeled with precision.</p>
            </div>
          </div>
        </div>
        <div style={{ backgroundColor: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '8px', padding: '14px 16px' }}>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
            ...
            <div>
              <p style={{ fontSize: '13px', fontWeight: 600, color: '#1d4ed8', margin: '0 0 4px' }}>Tip</p>
              <p style={{ fontSize: '13px', color: '#1e3a8a', margin: 0, lineHeight: 1.5 }}>Watch the full event before selecting a label. Context matters for accurate labeling.</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            <thead>
              <tr style={{ backgroundColor: '#f9fafb' }}>
                <th style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 600, color: '#374151', borderBottom: '1px solid #e5e7eb', width: '50%' }}>Focus On</th>
                <th style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 600, color: '#374151', borderBottom: '1px solid #e5e7eb', width: '50%' }}>Don't Focus On</th>
              </tr>
            </thead>
            <tbody>
              {[
...
export default function LearningHub() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      {/* Header */}
      <div>
        <h1 style={{ fontSize: '26px', fontWeight: 700, color: '#111827', margin: '0 0 4px' }}>Learning Hub</h1>
        <p style={{ fontSize: '14px', color: '#6b7280', margin: 0 }}>Study materials, references, and training resources.</p>
      </div>
      ...
      {activeTab === 'guidelines' && (
        <div style={card}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
            ...
            <span style={{ fontSize: '12px', color: '#9ca3af' }}>{page + 1} of {totalPages}</span>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '24px', paddingTop: '16px', borderTop: '1px solid #f3f4f6' }}>
            ...
            <button
              ...
              style={{
                display: 'inline-flex', alignItems: 'center', gap: '6px',
                padding: '7px 14px', fontSize: '13px', fontWeight: 600,
                backgroundColor: page === totalPages - 1 ? '#f9fafb' : '#6366f1',
                color: page === totalPages - 1 ? '#d1d5db' : 'white',
      ...
      {activeTab === 'references' && (
        <div style={card}>
          <h2 style={{ fontSize: '16px', fontWeight: 700, color: '#111827', margin: '0 0 16px' }}>Reference Materials</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {[
              ...
              <div key={ref.title} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', border: '1px solid #e5e7eb', borderRadius: '8px' }}>
                <div>
                  <p style={{ fontSize: '14px', fontWeight: 600, color: '#111827', margin: '0 0 2px' }}>{ref.title}</p>
                  <p style={{ fontSize: '12px', color: '#6b7280', margin: 0 }}>{ref.desc}</p>
                </div>
                <button style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '6px 12px', fontSize: '12px', fontWeight: 600, color: '#6366f1', backgroundColor: '#f0f4ff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>
                  View
      ...
      {activeTab === 'videos' && (
        <div style={card}>
          <h2 style={{ fontSize: '16px', fontWeight: 700, color: '#111827', margin: '0 0 16px' }}>Training Videos</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {[
              ...
              <div key={video.title} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 16px', border: '1px solid #e5e7eb', borderRadius: '8px' }}>
                <div style={{ width: '44px', height: '44px', backgroundColor: '#f0f4ff', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#6366f1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <polygon points="5 3 19 12 5 21 5 3"/>
                  </svg>
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: '14px', fontWeight: 600, color: '#111827', margin: '0 0 2px' }}>{video.title}</p>
                  <p style={{ fontSize: '12px', color: '#6b7280', margin: 0 }}>{video.duration}</p>
                </div>
                <button style={{ padding: '6px 12px', fontSize: '12px', fontWeight: 600, color: '#6366f1', backgroundColor: '#f0f4ff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>
                  Watch
                </button>
              </div>
            ))}
