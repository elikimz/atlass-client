...
export default function AdminUsers() {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '400px' }}>
        <div className="loading-container">
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '13px', fontWeight: 500, margin: 0 }}>Loading users...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div>
        <h1 style={{ fontSize: '26px', fontWeight: 700, color: 'var(--text-heading)', margin: '0 0 4px' }}>Manage Users</h1>
        <p style={{ fontSize: '14px', color: 'var(--text-muted)', margin: 0 }}>View and manage user accounts, roles, and status</p>
      </div>
              ...
              <tr key={user.id} style={{ borderBottom: '1px solid var(--border-main)', backgroundColor: user.is_suspended ? 'rgba(225, 29, 72, 0.05)' : 'transparent' }}>
                ...
                <td style={{ padding: '12px 16px', fontSize: '14px' }}>
                  {editingId === user.id ? (
                    ...
                    <span style={{ padding: '4px 8px', backgroundColor: user.role === 'admin' ? '#FEF3C7' : '#DBEAFE', color: user.role === 'admin' ? '#92400E' : '#1E40AF', borderRadius: '4px', fontSize: '12px', fontWeight: 600 }}>{user.role}</span>
                  )}
                </td>
                <td style={{ padding: '12px 16px', fontSize: '14px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <span style={{ padding: '2px 6px', backgroundColor: user.is_trained ? 'rgba(34, 197, 94, 0.1)' : 'var(--bg-main)', color: user.is_trained ? '#166534' : 'var(--text-muted)', borderRadius: '4px', fontSize: '11px', fontWeight: 600, width: 'fit-content' }}>{user.is_trained ? 'Trained' : 'Not Trained'}</span>
                    <span style={{ padding: '2px 6px', backgroundColor: user.is_suspended ? 'rgba(220, 38, 38, 0.1)' : 'rgba(34, 197, 94, 0.1)', color: user.is_suspended ? '#991B1B' : '#166534', borderRadius: '4px', fontSize: '11px', fontWeight: 600, width: 'fit-content' }}>{user.is_suspended ? 'Suspended' : 'Active'}</span>
                  </div>
                </td>
                ...
                <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                  {editingId === user.id ? (
                    <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}><button onClick={handleSave} style={{ padding: '6px 12px', backgroundColor: '#10B981', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 600, fontSize: '12px' }}>Save</button><button onClick={handleCancel} style={{ padding: '6px 12px', backgroundColor: 'var(--bg-main)', color: 'var(--text-main)', border: '1px solid var(--border-main)', borderRadius: '6px', cursor: 'pointer', fontWeight: 600, fontSize: '12px' }}>Cancel</button></div>
                  ) : (
                    <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}>
                      <button onClick={() => toggleSuspension(user)} style={{ padding: '6px 12px', backgroundColor: user.is_suspended ? 'rgba(34, 197, 94, 0.1)' : 'rgba(225, 29, 72, 0.1)', color: user.is_suspended ? '#166534' : '#E11D48', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 600, fontSize: '12px' }}>{user.is_suspended ? 'Unsuspend' : 'Suspend'}</button>
                      <button onClick={() => handleEdit(user)} style={{ padding: '6px 12px', backgroundColor: 'var(--accent-light)', color: 'var(--accent-primary)', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 600, fontSize: '12px' }}>Edit</button>
                      <button onClick={() => handleDelete(user.id)} style={{ padding: '6px 12px', backgroundColor: 'rgba(220, 38, 38, 0.1)', color: '#DC2626', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 600, fontSize: '12px' }}>Delete</button>
                    </div>
                  )}
                </td>
              </tr>
